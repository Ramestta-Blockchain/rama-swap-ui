...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.RAMA, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.RAMA]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.RAMA, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.RAMA]).toEqual(3)
    })
    it('works with non-set chains', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.TRAMA, blockNumber: 2 }))
